...
public abstract class ReactMapperBase : IComponentMapper
{
  ...
  /// <summary>Returns className="prismatic-{id}" (JSX uses className, not class).</summary>
  protected static string NodeClass(IRNode node) => $" className=\"prismatic-{node.Id}\"";
  ...
  // ── JSX tag builder helpers ───────────────────────────────────────────────

  /// <summary>Self-closing JSX element: &lt;Tag attrs /&gt;</summary>
  protected static string SelfClosing(string tag, string attrs, string indent) =>
      $"{indent}<{tag}{attrs} />\n";

  /// <summary>Paired JSX element: &lt;Tag attrs&gt;inner&lt;/Tag&gt;</summary>
